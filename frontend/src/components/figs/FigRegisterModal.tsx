...
export interface FigCreateForm {
  ...
  member_ids: string[]; president_farmer_id: string; president_mobile: string; president_password: string;
}
        ...
        <form onSubmit={onSubmit} className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {isStateAdmin ? (
            <div><label className="label-tag">District</label>
              <select required className="input mt-1" value={form.district_id} onChange={(e) => setForm({ ...form, district_id: e.target.value, seri_circle_id: "", member_ids: [], president_farmer_id: "", president_mobile: "" })}>
              ...
              {unassignedFarmers.map((f) => (
                <label key={f.id} className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={form.member_ids.includes(f.id)} onChange={(e) => {
                    ...
                    setForm({ ...form, member_ids: next, president_farmer_id: presStillIn ? form.president_farmer_id : "", president_mobile: presStillIn ? form.president_mobile : "" });
                  }} />
          ...
          {form.member_ids.length > 0 && (
            <div className="col-span-full border-t pt-4" style={{ borderColor: "var(--border)" }}>
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                <select className="input" value={form.president_farmer_id} onChange={(e) => {
                  const fr = unassignedFarmers.find((x) => x.id === e.target.value);
                  setForm({ ...form, president_farmer_id: e.target.value, president_mobile: fr?.mobile_no || "" });
                }}>
                ...
                <input placeholder="Login mobile" className="input" value={form.president_mobile} onChange={(e) => setForm({ ...form, president_mobile: e.target.value })} />
                <input placeholder="Password" type="password" className="input" value={form.president_password} onChange={(e) => setForm({ ...form, president_password: e.target.value })} />
              </div>
            </div>
          )}
